...
export default function UserProfile() {
  ...
  if (!data) return <div className="p-10 text-center text-amber-800">Loading…</div>;
  ...
  return (
    <div className="p-6">
      <div className="flex items-center gap-4 mb-6">
        {user.avatar_url && (
          <img src={user.avatar_url} alt="" className="w-16 h-16 rounded-full border border-amber-200" />
        )}
        <div>
          <h1 className="text-2xl font-bold text-amber-900">{user.display_name}</h1>
              ...
              <div className="flex items-start gap-3">
                {t.coffee?.image_url && (
                  <img src={t.coffee.image_url} alt=""
                       className={`w-16 h-16 rounded object-cover border border-amber-100 flex-shrink-0 ${t.coffee.is_removed ? 'grayscale' : ''}`} />
                )}
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start gap-3">
                    <div className="min-w-0">
                      {t.coffee ? (
                        <Link to={`/c/${t.coffee.id}`}
                              className={`text-amber-900 font-medium hover:underline ${t.coffee.is_removed ? 'line-through' : ''}`}>
                          {t.coffee.name}
                        </Link>
                      ) : (
                        <span className="text-amber-900 font-medium">Coffee removed</span>
                      )}
                    ...
                    <div className="text-amber-700 text-lg flex-shrink-0">
